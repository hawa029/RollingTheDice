	...
	.addEventListener('click', function () {
		if (startGame) {
			...
			switch (randomNumber) {
				...
				default:
					zone.style.backgroundImage = false;
			}

			if (randomNumber !== 1) {
				//Add score
				pointGot += randomNumber;
				document.querySelector('#round-' + currentPlayer).textContent =
					pointGot;
			} else {
				pointGot = 0;
				nextPlayer();
...
holdScore.addEventListener('click', function () {
	if (startGame) {
		...
		if (scores[currentPlayer] >= 100) {
			document.querySelector('#player-' + currentPlayer).innerHTML =
				'<h4>Vainqueur !</h4>';
			document.querySelector('.winner-annunce').innerHTML =
